...
(function($) {	
	...
	function getPosition(trigger, el, conf) {	
		...
		var top = trigger.offset().top, 
			...
			pos = getTriggerOptions(trigger,'position');
		if(pos === false)
			pos = conf.position;
		var y = pos[0],
			x = pos[1];
	...
	function getTriggerOptions(trigger , field)
	{
		var trigger_options = $.parseJSON(trigger.attr('options'));
		if( trigger_options != null)
		{
			if(field != 'undefined')
				if(trigger_options[field] != null)
					return trigger_options[field];
		}
		return false;
	}
		...
		'default' : [
			...
			function(errs) {
				...
				$.each(errs, function(i, err) {
					...
					if (!msg) { 
						msg = $(conf.message).addClass(conf.messageClass).addClass('tooltip_holder').appendTo(document.body);
		...
		'tooltips' : [
			...
			function(errs) {
				...
				$.each(errs, function(i, err) {
					...
					if (!msg)
					{
						msg = self.createTip(input,message);
						input.data("msg.el", msg);
					}
	...
	*/
	...
	v.fn(":email", "Please enter a valid email address", function(el, v) {return !v || emailRe.test(v);});
	v.fn(":url", "Please enter a valid URL", function(el, v) {return !v || urlRe.test(v);});
	v.fn(":number", "Please enter a numeric value.", function(el, v) {return numRe.test(v);});
	...
	v.fn("[min]", "Please enter a value greater than $1", function(el, v) {
	...
	v.fn("[required]", "Please complete this field.", function(el, v) {
	...
	v.fn("[data-equals]", "Value not equal with the $1 field", function(input) {
		var name = input.attr("data-equals"),
		 field = this.getInputs().filter("[name=" + name + "]"); 
		return input.val() == field.val() ? true : [name]; 
	});
	
	v.fn("[minlength]", "Please provide at least $1 character(s). ",function(input, value) {
		var min = input.attr("minlength");
		return value.length >= min ? true : [min];
	});
	
	v.fn("[maxlength]", "Please provide max $1 character(s). ",function(input, value) {
		var max = input.attr("maxlength");
		return value.length <= min ? true : [max];
	});
	...
	function Validator(inputs, form, conf) {		
		...
		$.extend(self, {
			...
			createTip : function (trigger,messages)
			{
				var cssClass = null;
					pos = getTriggerOptions(trigger,'position');
				if(pos === false)
				...
				var tip_holder = $(conf.message).addClass(conf.tipClass+conf.otherClass.holder).appendTo(document.body).css({'display':"block"});
				if(typeof pos[0] != 'undefined' && typeof pos[1] != 'undefined')
					cssClass = pos[0]+'_'+pos[1];
				...
				var tip_content = tip_holder.children().eq(1).html(messages).addClass(conf.tipClass+conf.otherClass.content);
				if(conf.hasArrow)
					tip_holder = this.createArrow(pos,tip_holder);
				return tip_holder;
			},
			
			createArrow : function(pos,tip_holder)
			{
				...
				var new_top = 0,
					new_left = 0,
					holder_width = tip_holder.width() + (tip_holder.outerWidth() - tip_holder.innerWidth()),
				...
				if(typeof pos[0] != 'undefined')
				{
					...
					else if(pos[0] === 'center')
					{
						...
						{
							if(pos[1] === 'left')
								new_left = holder_width + offset +1;
							else if(pos[1] === 'center')
							{
								new_top = 0;
								new_left = 0;
...
//}}} 
		...
		// form reset
		form.bind("reset.V", function(){self.reset();});
		...
		var radios = inputs.filter(":radio").change(function(e) {self.checkValidity(radios, e);});
		// reposition tooltips when window is resized
		$(window).resize(function() {self.reflow();});
	}
